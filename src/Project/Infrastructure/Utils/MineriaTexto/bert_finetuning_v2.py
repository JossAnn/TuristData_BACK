...
class BERTSentimentRegressor(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids=None):
        ...
        for category in ['atencion', 'limpieza', 'precio', 'comida']:
            ...
            presence_prob = self.category_modules[category]['presence_classifier'](pooled_output).squeeze(-1)
            
            # Puntuación (aplicar sigmoid y escalar a rango 1-5)
            raw_score = self.category_modules[category]['score_regressor'](pooled_output).squeeze(-1)
...
class SentimentDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        processed_labels = {}
        for category in ['atencion', 'limpieza', 'precio', 'comida']:
            if pd.isna(self.labels[idx][category]) or self.labels[idx][category] is None:
                ...
                processed_labels[category] = {
                    ...
                    'score': torch.tensor(float(self.labels[idx][category]), dtype=torch.float),
                    'has_label': True
                }
...
def compute_loss(predictions, labels, presence_weight=1.0, score_weight=1.0):
    ...
    for category in ['atencion', 'limpieza', 'precio', 'comida']:
        # Obtener máscaras para muestras que tienen etiquetas válidas
        batch_size = len(labels)
        valid_mask = torch.tensor([labels[i][category]['has_label'] for i in range(batch_size)])
        
        if valid_mask.sum() > 0:  # Si hay al menos una muestra válida
            # Pérdida de detección de presencia (para todas las muestras)
            pred_presence = predictions[category]['presence_prob']
            true_presence = torch.stack([labels[i][category]['present'] for i in range(batch_size)])
            
            if pred_presence.device != true_presence.device:
                true_presence = true_presence.to(pred_presence.device)
            ...
            present_and_valid_mask = valid_mask & (true_presence > 0.5)
            
            if present_and_valid_mask.sum() > 0:
                pred_scores = predictions[category]['score'][present_and_valid_mask]
                true_scores = torch.stack([labels[i][category]['score'] for i in range(batch_size)])[present_and_valid_mask]
                
                if pred_scores.device != true_scores.device:
                    true_scores = true_scores.to(pred_scores.device)
                
                score_loss = mse_criterion(pred_scores, true_scores)
    ...
    for _, row in df.iterrows():
        ...
        for category in ['atencion', 'limpieza', 'precio', 'comida']:
            if pd.isna(row[category]):
                ...
                label_dict[category] = float(row[category])
...
def main():
    ...
    # CAMBIA 'tu_dataset.csv' por la ruta a tu archivo CSV
    CSV_PATH = 'opiniones.csv'  # <-- CAMBIAR ESTA RUTA
    ...
    try:
        ...
        for category, count in category_counts.items():
            print(f"  {category}: {count} ejemplos ({count/len(texts)*100:.1f}%)")
            
    except FileNotFoundError:
        print(f"Error: No se encontró el archivo {CSV_PATH}")
        print("Usando datos de ejemplo...")
        # Fallback a datos de ejemplo si no encuentra el CSV
        sample_texts = [
            "El servicio fue excelente pero la comida regular",
            "Lugar limpio pero caro",
            "Servicio regular, comida buena",
            "Excelente atención, muy limpio",
            "Mala atención, lugar sucio"
        ]
        sample_labels = [
            {'atencion': 5, 'limpieza': None, 'precio': None, 'comida': 2},
            {'atencion': None, 'limpieza': 5, 'precio': 1, 'comida': None},
            {'atencion': 3, 'limpieza': None, 'precio': None, 'comida': 4},
            {'atencion': 5, 'limpieza': 5, 'precio': None, 'comida': None},
            {'atencion': 1, 'limpieza': 1, 'precio': None, 'comida': None}
        ]
    ...
    config = {
        ...
        "presence_threshold": 0.5  # Umbral para considerar que una categoría está presente
    }
...
def load_and_predict(model_path, text, presence_threshold=0.5):
...
def predict_example():
    ...
    try:
        ...
        for text in texts:
            ...
            for category, result in predictions.items():
                if result is not None:
                    print(f"  {category.capitalize()}: {result['score']}/5 (confianza: {result['confidence']})")
                else:
        ...
        averages = calculate_average_scores(all_predictions)
        for category, avg_data in averages.items():
            if avg_data is not None:
                print(f"{category.capitalize()}: {avg_data['average']}/5 (basado en {avg_data['count']} menciones)")
            else:
