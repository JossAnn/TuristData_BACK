...
def compute_loss(predictions, labels, presence_weight=1.0, score_weight=1.0):
    ...
    for category in ["atencion", "limpieza", "precio", "comida"]:
        ...
        valid_mask = torch.tensor(
            [labels[i][category]["has_label"] for i in range(batch_size)]
        )
        ...
        if valid_mask.sum() > 0:  # Si hay al menos una muestra válida
            ...
            true_presence = torch.stack(
                [labels[i][category]["present"] for i in range(batch_size)]
            )
            ...
            if present_and_valid_mask.sum() > 0:
                ...
                true_scores = torch.stack(
                    [labels[i][category]["score"] for i in range(batch_size)]
                )[present_and_valid_mask]
                ...
                category_loss = (
                    presence_weight * presence_loss + score_weight * score_loss
                )
            else:
...
def main():
    ...
    CSV_PATH = "tu_dataset.csv"  # <-- CAMBIAR ESTA RUTA
