        ...
        for category in ['atencion', 'limpieza', 'precio', 'comida']:
            ...
            if pd.isna(value) or value == '' or (isinstance(value, str) and value.strip() == ''):
                ...
                try:
                    ...
                    print(f"Advertencia: No se pudo convertir '{value}' a número en categoría '{category}'. Se tratará como nulo.")
                    label_dict[category] = None
    ...
    try:
        ...
        for category, count in category_counts.items():
            if count > 0:
                ...
                print(f"  {category}: {count} ejemplos ({count/len(texts)*100:.1f}%) - Promedio: {avg_score:.2f}")
            else:
